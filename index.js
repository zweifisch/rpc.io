  ...
  module.exports = function(socket) {
      ...
      if (args.length === 1) {
        ...
        if ('function' === typeof args[1]) {
          ...
          for (method in methods) {
            if (!hasProp.call(methods, method)) continue;
            closure = methods[method];
            closures[namespace + "." + method] = closure;
            if (methods[method + "_defaults"]) {
              results.push(defaults[namespace + "." + method] = methods[method + "_defaults"]);
            } else {
              results.push(void 0);
            }
          }
    ...
    rpchandler = function(method, params, id) {
      ...
      for (name in params) {
        ...
        value = params[name];
        if (indexOf.call(signatures[method], name) < 0) {
          throw new Error("unexpected param: " + name);
        }
      }
      ref = signatures[method];
      ...
      try {
        ...
        if ('function' === typeof (result != null ? result.then : void 0)) {
          ...
          return result["catch"](function(err) {
            return socket.emit('rpc-result', id, null, (err != null ? err.message : void 0) || err);
          });
        } else {
